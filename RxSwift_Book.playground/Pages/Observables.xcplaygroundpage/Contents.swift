 ...
 * Observable은 이벤트 시퀀스를 생성하고 발생시키는(emit) 기능을 가지고 있으며, 이렇게 발생한 이벤트들은 인스턴스나 탭과같은 제스처를 인식할 수도 있습니다.
 ...
 * Subscribe호출과 동시에 기존의 이벤트를 방출하기 시작하는 Hot Observable과 Subscribe호출 이후부터 받은 이벤트를 방출하는 Cold Observable로 나뉘어집니다.
